...
from ai.matching import find_similar_found_posts
...
st.set_page_config(page_title="찾아요 - 명지 스마트 분실물 센터", page_icon="🔍", layout="wide")

auth.render_sidebar_auth()

st.title("🔍 찾아요 게시판")
st.caption("잃어버린 물건을 등록하고 찾아보세요.")

user = auth.require_ready_user("찾아요 게시판을 이용하려면")
if user is None:
    st.stop()
user_id = user["id"]
...
# ---------------- 목록 + 상세 ----------------
with tab_list:
    with st.form("lost_search_form"):
        search_mode = st.radio(
            "검색 방식", ["키워드 검색", "AI 의미 검색"], horizontal=True, key="lost_search_mode"
        )
        c1, c2, c3 = st.columns([2, 1, 1])
        if search_mode == "키워드 검색":
            keyword = c1.text_input("검색어", placeholder="예: 에어팟", key="lost_keyword_input")
            status_options = ["전체", "찾는 중", "찾음"]
        else:
            keyword = c1.text_input(
                "검색어 (문장으로 입력해보세요)",
                placeholder="예: 검은색 에어팟을 도서관에서 잃어버렸어요",
                key="lost_ai_query_input",
            )
            status_options = ["전체", "보관 중", "완료"]  # AI 모드는 반대편(습득물) 게시판을 검색함
        category_filter = c2.selectbox("카테고리", ["전체"] + CATEGORIES, key="lost_category_filter")
        status_filter = c3.selectbox("상태", status_options, key="lost_status_filter")
        submitted = st.form_submit_button("검색")

    if search_mode == "키워드 검색":
        try:
            posts = db.search_lost_posts(
        ...
        for post in posts:
            with st.container(border=True):
                ...
                with c1:
                    ...
                    st.caption(
                        f"{post['category']} · {post['location']} · {post['lost_at']} · 상태: {post['status']}"
                    )
                    st.caption(f"작성자: {post['author_nickname']}")
                with c2:
                    if st.button("상세보기", key=f"lost_detail_btn_{post['id']}"):
                        st.session_state["selected_lost_id"] = post["id"]
                    # 새 탭에서 여는 링크 -- 클릭해도 현재 탭의 검색/목록
                    # 상태는 전혀 건드리지 않는다. 새 탭은 session_state가
                    # 없는 새 세션으로 열리므로, 상세 정보는 아래 query
                    # params(lost_id)를 통해 복원한다.
                    st.link_button(
                        "🔗 새 탭에서 보기",
                        f"?lost_id={post['id']}",
                        key=f"lost_detail_newtab_{post['id']}",
                    )

    else:  # AI 의미 검색: 문장으로 입력한 특징과 의미가 비슷한 "찾았어요"(습득물) 게시물을 찾음
        ai_state_key = "lost_ai_search_results"
        if submitted:
            ...
            if not query:
                ...
                with st.spinner(
                    "AI가 의미가 비슷한 습득물을 찾는 중입니다... "
                    "(모델을 처음 불러오는 경우 다운로드 때문에 다소 시간이 걸릴 수 있어요)"
                ):
                    try:
                        candidates = db.search_found_posts(
                            category=None if category_filter == "전체" else category_filter,
                            status=None if status_filter == "전체" else status_filter,
                        )
    ...
    if st.session_state.get("selected_lost_id") is None:
        # 새 탭으로 막 열린 경우(session_state 없음) query param으로 복원.
        # 이미 이 탭에서 선택된 게시물이 있으면 URL을 신뢰하지 않는다.
        query_lost_id = st.query_params.get("lost_id")
        if query_lost_id is not None:
            try:
                st.session_state["selected_lost_id"] = int(query_lost_id)
            except (TypeError, ValueError):
                pass  # 잘못된 형식의 ID -- 무시하고 목록만 정상 표시
    selected_id = st.session_state.get("selected_lost_id")
    if selected_id:
        st.divider()
        try:
            post = db.get_lost_post(selected_id)
        except Exception as e:
            st.error(f"게시물을 불러오는 중 오류가 발생했습니다: {e}")
            post = None
        ...
        if post is None:
            ...
            # 만들지 않는다 -- get_or_create_direct_chat_room() 참고.
            if st.button("💬 작성자와 채팅하기", key=f"direct_chat_btn_lost_{post['id']}"):
                try:
                    room = db.get_or_create_direct_chat_room("lost", post["id"], user_id)
            ...
            st.divider()
            # LostPost ids are used as-is (positive) for target_type="post";
            # pages/2_찾았어요.py negates FoundPost ids -- see db.create_report().
            render_report_control("post", post["id"])

            st.divider()
            render_ai_match_section(
                post=post,
                button_label="🤖 AI로 유사한 습득물 찾기",
                find_similar_fn=find_similar_found_posts,
                list_candidates_fn=db.list_found_posts,
                result_kind="found",
                target_page="pages/2_찾았어요.py",
                selected_session_key="selected_found_id",
            )

# ---------------- 새 글 등록 ----------------
with tab_new:
    st.write("잃어버린 물건 정보를 입력해주세요. (* 필수)")
    with st.form("lost_new_form", clear_on_submit=True):
        title = st.text_input("제목 *")
        description = st.text_area("설명 *")
        category = st.selectbox("카테고리 *", CATEGORIES, key="lost_new_category")
        location = st.text_input("분실 장소 *", placeholder="예: 인문캠퍼스 도서관")
        col_d, col_t = st.columns(2)
        lost_date = col_d.date_input("분실 날짜 *")
        lost_time = col_t.time_input("분실 시간 *")
    ...
    if submitted:
        ...
        if errors:
            ...
            try:
                ...
                try:
                    new_id = db.create_lost_post(
                        user_id=user_id,
                        title=title.strip(),
                        description=description.strip(),
                        category=category,
                        location=location.strip(),
                        lost_at=format_datetime_input(lost_date, lost_time),
                        image_url=image_url,
                    )
                except db.PermissionDeniedError as e:
                    st.error(str(e))
                    ...
                    st.session_state["selected_lost_id"] = new_id
