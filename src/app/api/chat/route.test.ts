...
vi.mock("@/lib/chat/service", () => ({ getOrCreateDirectChatRoom, listChatRoomsForUser }));
...
describe("POST /api/chat", () => {
  ...
  // Phase J-2: { matchId } was the other accepted body shape until the
  // Match domain was removed; it's now just an invalid body like any other.
  it("rejects a { matchId } body now that Match is gone", async () => {
    requireUserForApi.mockResolvedValueOnce({ user: sessionUser });
  ...
  // Phase 10: the { postType, postId } body shape creates/returns the
  // direct room with that post's author.
  describe("direct chat body ({ postType, postId })", () => {
    ...
    it("rejects a body missing postId", async () => {
      requireUserForApi.mockResolvedValueOnce({ user: sessionUser });
