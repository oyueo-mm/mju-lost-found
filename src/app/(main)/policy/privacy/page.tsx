...
const SECTIONS: Section[] = [
  ...
  },
  {
    ...
    body: (
      <>
        회원 탈퇴 전까지는 계정 정보를 계속 보유합니다. 회원 탈퇴 시 처리 방식은 아래 6조를 따르며,
        게시글·댓글·채팅 메시지는 탈퇴 여부와 무관하게 이용자가 직접 삭제하기 전까지 보유될 수
        있습니다. 별도의 법적 보존 의무가 있는 경우는 이 서비스의 코드로 확인되지 않아 안내하지
        않습니다.
      </>
    ),
  },
  ...
  },
  {
    ...
    body: (
      <>
        ...
        <p className="mt-2">
          회원 탈퇴는 내 정보 화면에서 신청할 수 있습니다. 탈퇴 시 이메일, 이름, 닉네임, Google 계정
          연결 정보는 복구할 수 없도록 익명 처리되며, 계정으로 다시 로그인할 수 없게 됩니다. 다만
          작성하신 게시글·댓글·채팅 메시지 자체는 삭제되지 않고 그대로 남으며, 작성자는
          &quot;탈퇴한 사용자&quot;로 표시됩니다. 이는 다른 이용자가 이미 주고받은 대화나 게시물의
          맥락이 갑자기 사라지지 않도록 하기 위한 것입니다. 탈퇴 시 알림 내역은 함께 삭제됩니다.
        </p>
      </>
    ),
  },
  {
    ...
    body: (
      <>
        회원 탈퇴 시 이메일·이름·닉네임·Google 계정 연결 정보는 복구할 수 없도록 즉시 익명 처리되고,
        알림 내역은 삭제됩니다. 본인이 직접 삭제한 채팅 메시지에 첨부된 사진은 저장소에서도 함께
        삭제됩니다. 다만 그 외의 경우(예: 관리자가 신고 처리로 숨긴 게시물·메시지, 신고·제재 기록)는
        운영 기록 보전을 위해 별도로 파기하지 않습니다. 이 서비스에는 일정 기간이 지나면 자동으로
        데이터를 파기하는 절차는 구현되어 있지 않습니다.
      </>
    ),
  },
  {
