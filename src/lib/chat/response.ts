// Split from http.ts for the same reason as posts/response.ts: no
// next-auth import chain, so tests can import this directly without
// pulling in next-auth (which doesn't resolve under Vitest's plain Node
// ESM outside of Next's own bundler).
