...
class NaturalLanguageSearchUiTestCase(unittest.TestCase):
    ...
    # ---------- board label on AI search result cards ----------

    def test_lost_board_ai_search_result_labeled_as_found_board(self):
        found_post = db.get_found_post(self.found_id)
        fake_result = [MatchCandidate(post=found_post, score=0.91)]
        ...
        captions = [c.value for c in at.caption]
        self.assertTrue(
            any("찾았어요 게시판" in c for c in captions),
            "찾아요 게시판 AI 검색 결과 안내에 '찾았어요 게시판'이라는 문구가 있어야 함",
        )
        self.assertTrue(
            any("찾았어요 게시글" in c for c in captions),
            "결과 카드 자체에도 게시판 종류(찾았어요 게시글) 라벨이 있어야 함",
        )

    def test_found_board_ai_search_result_labeled_as_lost_board(self):
        lost_post = db.get_lost_post(self.lost_id)
        fake_result = [MatchCandidate(post=lost_post, score=0.88)]
        ...
        captions = [c.value for c in at.caption]
        self.assertTrue(
            any("찾아요 게시판" in c for c in captions),
            "찾았어요 게시판 AI 검색 결과 안내에 '찾아요 게시판'이라는 문구가 있어야 함",
        )
        self.assertTrue(
            any("찾아요 게시글" in c for c in captions),
            "결과 카드 자체에도 게시판 종류(찾아요 게시글) 라벨이 있어야 함",
        )


class KeywordSearchBoardIsolationUiTestCase(unittest.TestCase):
    """회귀 테스트: 두 게시판에 동일한 키워드가 포함된 게시글이 있어도,
    키워드 검색 결과에는 자기 게시판 게시글만 나와야 한다 (반대편 게시판
    게시글은 절대 섞이면 안 됨). setUp()의 lost/found 픽스처는 일부러
    "무선이어폰"이라는 공통 키워드를 제목에 포함시켜, 검색 결과가
    조회하는 테이블을 헷갈리면 바로 드러나도록 구성했다."""

    def setUp(self):
        self._tmp_db = Path(__file__).resolve().parent / "_tmp_keyword_isolation_ui.db"
        self._orig_db_path = db.DB_PATH
        db.DB_PATH = self._tmp_db
        if self._tmp_db.exists():
            self._tmp_db.unlink()
        ...
        self.uid = db.create_user("student2@mju.ac.kr", "학생2")
        db.set_initial_nickname(self.uid, "학생2닉네임")
        self.lost_id = db.create_lost_post(
            self.uid, "검은색 무선이어폰 분실", "케이스에 흰색 스티커", "전자기기", "인문캠퍼스 도서관", "2026-08-25 15:00"
        )
        self.found_id = db.create_found_post(
            self.uid, "검은색 무선이어폰 습득", "케이스에 흰색 스티커 있음", "전자기기", "인문캠퍼스 도서관", "2026-08-25 16:00"
        )
    ...
    def test_lost_board_keyword_search_excludes_found_posts(self):
        at = AppTest.from_file(LOST_PAGE)
        at.run(timeout=30)
        # default mode is already "키워드 검색"
        at.text_input(key="lost_keyword_input").set_value("무선이어폰")
        at.button(key="FormSubmitter:lost_search_form-검색").click()
        at.run(timeout=30)

        self.assertEqual(list(at.exception), [])
        markdowns = [m.value for m in at.markdown]
        self.assertTrue(any("검은색 무선이어폰 분실" in m for m in markdowns))
        self.assertFalse(any("검은색 무선이어폰 습득" in m for m in markdowns))

    def test_found_board_keyword_search_excludes_lost_posts(self):
        at = AppTest.from_file(FOUND_PAGE)
        at.run(timeout=30)
        at.text_input(key="found_keyword_input").set_value("무선이어폰")
        at.button(key="FormSubmitter:found_search_form-검색").click()
        at.run(timeout=30)

        self.assertEqual(list(at.exception), [])
        markdowns = [m.value for m in at.markdown]
        self.assertTrue(any("검은색 무선이어폰 습득" in m for m in markdowns))
        self.assertFalse(any("검은색 무선이어폰 분실" in m for m in markdowns))
